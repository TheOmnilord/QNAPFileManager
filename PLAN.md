...
9. **Recursion crosses storage domains, not devices.** QuTS hero mounts one ZFS dataset per shared folder, so `st_dev` differs per share and the rev 1 one-filesystem rule would skip every share. Walks may cross into mounts of the same domain (`zfs:<pool>` or the same block device) that are `Storage` and not `Network`; they still never enter `/proc`, `/sys`, `/dev`, tmpfs, USB disks, other pools or network mounts. **Amended 2026-09-23 (hardware report: a search of `/share` for `QKVM` returned nothing):** a walk that only reads (search and folder size, by explicit opt-in: `WalkOptions.ReadCrossing` / `Platform.MayCrossRead`) may also step from a parent that is not storage at all into a `Storage`, non-`Network` child. `/share` is a tmpfs RAM disk with every volume mounted under it, so searching `/share` is the natural way to search every volume and there is no domain on the parent side to protect; once inside a storage mount the same-domain rule governs every further crossing. Delete, trash, chmod/chown, copy/move, archive, every pre-scan that confirms one and the Permissions dialog's impact estimate keep the strict rule (the client states the measurement's purpose, so a relaxed total is never reused as a chmod estimate). A search or size says how many local storage mount points it did not enter (pseudo-filesystems, tmpfs and unidentifiable mounts are not counted: none can be searched directly) and, separately, how many network shares it refused, which no search can enter. Exposed in the UI as "Include mounted sub-folders"; the search dialog offers it on QTS as well as hero (owner, 2026-09-23: on QTS too every volume is a mount under the RAM disk), while the delete, copy, archive and permissions dialogs keep it hero-only. Moves between shares on hero are `EXDEV`: predicted before the job starts and stated in the confirm dialog; `EDQUOT` gets its own message.

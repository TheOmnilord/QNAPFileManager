...
func TestSizeAppliesTheCrossingRule(t *testing.T) {
	...
	var off jobLog
	res, err := Size(context.Background(), r, plat, []string{api + "/a"}, SizeOptions{}, off.emit())
	if err != nil {
		t.Fatalf("Size: %v", err)
	}
	...
	var on jobLog
	res, err = Size(context.Background(), r, plat, []string{api + "/a"}, SizeOptions{CrossMounts: true}, on.emit())
	if err != nil {
		t.Fatalf("Size: %v", err)
	}
...
func TestSizeWarnsForAPathThatIsNotThere(t *testing.T) {
	...
	res, err := Size(context.Background(), r, nil, []string{"/nope", "/a/one.txt"}, SizeOptions{}, log.emit())
	if err != nil {
		t.Fatalf("Size: %v", err)
	}
	if res.Files != 1 {

...
- **§2.6 — audit result-write timeout under a wedged sink.** `audit.WriteSync` abandons its wait after `writeSyncTimeout` (2s) while the write goroutine keeps its slot until the underlying write+fsync finishes (so a stuck write is never abandoned back into the pool). If the sink is wedged for longer than that *during a read-only settings toggle*, the result write returns `ErrSyncTimeout` and the handler rolls the guard and config back to the previous value — yet the abandoned goroutine may still, seconds later, durably persist the `readOnly=newVal ok` line, leaving that late line uncompensated by a rollback record. This is accepted for M1: it needs an fsync stalled past 2s at the exact moment of a toggle, the live outcome is **fail-safe** (the guard and config are the previous value, so writes are actually still blocked — the log merely over-reports that they were enabled), and the durable *intent* line written before anything was applied already records that a toggle was attempted. The clean close is the same architectural change §2.0 defers (a durable write that is never abandoned while its goroutine runs); deferred past M1.

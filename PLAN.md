...
9. **Recursion crosses storage domains, not devices.** QuTS hero mounts one ZFS dataset per shared folder, so `st_dev` differs per share and the rev 1 one-filesystem rule would skip every share. Walks may cross into mounts of the same domain (`zfs:<pool>` or the same block device) that are `Storage` and not `Network`; they still never enter `/proc`, `/sys`, `/dev`, tmpfs, USB disks, other pools or network mounts. **Amended 2026-09-23 (hardware report: a search of `/share` for `QKVM` returned nothing):** a walk that only reads (search and folder size, by explicit opt-in: `WalkOptions.ReadCrossing` / `Platform.MayCrossRead`) may also step out of a "system" filesystem — the mount at `/` (decided from the mount row) or a RAM filesystem (tmpfs, ramfs) — into a RAM filesystem or a `Storage`, non-`Network` child. `/share` is a tmpfs RAM disk with every volume mounted under it, and `/` above it is a RAM disk too (a tmpfs on the TVS-h1688X, an ext2 RAM disk on some QTS), so searching `/share` or `/` is the natural way to search every volume and neither has a domain to protect. A search of `/` therefore also enters `/tmp`, `/var/log`, the pool roots and `/mnt/*` storage. Pseudo-filesystems (proc, sysfs, devtmpfs, cgroup …) and network mounts are never entered; once inside a storage mount other than `/` the same-domain rule governs every further crossing, so a tmpfs or USB disk inside a pool stays out (second hardware report, 2026-09-23: a search of `/` found only `/etc` symlinks into a pool). Delete, trash, chmod/chown, copy/move, archive, every pre-scan that confirms one and the Permissions dialog's impact estimate keep the strict rule (the client states the measurement's purpose, so a relaxed total is never reused as a chmod estimate). A search or size says how many local storage or RAM mount points it did not enter (a RAM disk such as `/share` may hold every volume; pseudo-filesystems and unidentifiable mounts are not counted) and, separately, how many network shares it refused, which no search can enter. Exposed in the UI as "Include mounted sub-folders"; the search dialog offers it on QTS as well as hero (owner, 2026-09-23: on QTS too every volume is a mount under the RAM disk), while the delete, copy, archive and permissions dialogs keep it hero-only. Moves between shares on hero are `EXDEV`: predicted before the job starts and stated in the confirm dialog; `EDQUOT` gets its own message.

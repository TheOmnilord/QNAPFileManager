...
- **§2.6 — audit result-write timeout under a wedged sink.** `audit.WriteSync` abandons its wait after `writeSyncTimeout` (2s) while the write goroutine keeps its slot until the underlying write+fsync finishes (so a stuck write is never abandoned back into the pool). If the sink is wedged for longer than that *during a read-only settings toggle*, the result write returns `ErrSyncTimeout` and the handler rolls back and tells the admin the change was refused — yet the abandoned goroutine may still, seconds later, durably persist the `readOnly=newVal ok` line, leaving that late line uncompensated by any rollback record. What is **guaranteed** is that the live guard and the persisted config never disagree: on a rollback the guard is set to whatever value the config file actually ends up holding — determined by *re-reading the file* when the rollback save reports an error, because a save error does not imply the file is unchanged (`config.Save` renames a temp file into place, so the rename can land while a later `SyncDir`/`Chmod` fails). So even a double failure (the sink wedged *and* the rollback save erroring, whether that error was before or after the rename) leaves the guard equal to the on-disk value. Only if the file is also unreadable at that instant (a triple failure) is the on-disk state unknowable; the guard is then forced **fail-closed** to read-only (writes blocked) and the discrepancy logged. The residual flaw is confined to the *audit narrative*: a late `ok` line can claim a toggle that was in fact rolled back (in either direction — reading as more or as less restrictive than the live state), and in the rare double-failure the applied value can differ from the "refused" response the admin was shown. This is accepted for M1: it needs an fsync stalled past 2s at the exact instant of a toggle, it never desynchronises the guard from the config, and the durable *intent* line written before anything was applied already records that a toggle was attempted. The clean close is the same architectural change §2.0 defers (a durable write that is never abandoned while its goroutine runs); deferred past M1.

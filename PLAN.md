...
6. **Administrators start as themselves.** An admin session runs a worker under the admin's own uid. "Act as administrator (root)" is an explicit per-session toggle, merged with the existing `systemWrite` arming switch: arming spawns a root worker (same code path, no `Credential`), TTL 60 min, disarms on restart, audited as both the QTS actor and the effective Linux identity. Root mode is what unlocks writes outside `/share`.
7. **Safety model** unchanged in shape: `readOnly` (global, persisted, default on at first install, password to disable), the guard's prefix rule table (applies to non-root sessions too as defence in depth, plus a never-descend component rule for `.zfs`), HMAC confirmation tokens carrying a real scan summary, audit JSON lines with intent and result phases mirrored to QuLog for milestones. Guard and audit live in the root front-end before the RPC.

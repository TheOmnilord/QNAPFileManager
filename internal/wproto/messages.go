...
type JobResult struct {
	...
	// MountsSkipped counts the local mount points a search or a size walk reached
	// and did not enter that are STORAGE or a RAM filesystem (tmpfs, ramfs): a
	// volume or pool, or /share itself — a RAM disk that may hold every volume.
	// The mount point itself is still visited (and a hit if its name matches);
	// only what is under it went unexamined. A pseudo-filesystem (/proc, /sys,
	// /dev, cgroup and the rest) or a mount the walk could not identify is not
	// counted: nobody can search inside it (Astra r2 on the QKVM fix, amended by
	// the "search of /" report). It is a count, not a list, so the frame stays
	// bounded, and omitempty keeps an older peer's frames byte-identical.
...
type SizeReq struct {
	...
	// decision 9, amended): from a system parent — "/" or a tmpfs such as /share —
	// the walk may enter a RAM filesystem or a storage volume. The folder-size
	// route sets it when the client asks (Properties does; the Permissions impact
	// estimate does not). The pre-scans that measure in order to confirm a change
	// (permissions, transfer) leave it off, so their count is the one the change
	// will walk.
